...
You could consider using native multi-lines instead of Code Fence Detection if
your example code does not contain any blank lines.

## Default Fallback

If code fencing is not detected, Docusaurus.Powershell will treat the first paragraph
of the example (all lines up to the first blank line) as code and everything below it
as the description, exactly matching the native Get-Help behavior described above.
